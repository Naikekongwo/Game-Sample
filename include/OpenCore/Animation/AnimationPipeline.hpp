...
class AnimationPipeline
{
  public:
    // 构造函数(弃用)
    explicit AnimationPipeline(AnimationManager *mgr) : manager(mgr) {}

    // 添加帧动画 : 是一个接口的实例
    // AnimationPipeline& Frame(uint8_t totalFrames, uint8_t FPS, bool isLooping
    // = false) {
    //     animations.push_back(std::make_shared<FrameAnimation>(totalFrames,
    //     FPS, isLooping)); return *this;
    // }

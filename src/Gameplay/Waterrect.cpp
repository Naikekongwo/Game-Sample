...
void Waterrect::Draw()
{
    ...
    // 懒创建 1x1 白色纹理：白色 × 顶点颜色 = 纯顶点色渲染，
    // 与原版（空纹理）效果一致，避免真实水面贴图造成异常高亮，
    // 且比传 nullptr 更稳妥（SDL_RenderGeometry(nullptr) 会卡死）。
    if (!solidWhiteTexture)
    {
        solidWhiteTexture = SDL_CreateTexture(
            renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC, 1, 1);
        if (solidWhiteTexture)
        {
            SDL_SetTextureBlendMode(solidWhiteTexture, SDL_BLENDMODE_BLEND);
            const uint32_t white = 0xFFFFFFFFu;
            SDL_UpdateTexture(solidWhiteTexture, nullptr, &white, 4);
        }
    }
    if (!solidWhiteTexture)
        return;
    ...
    SDL_RenderGeometry(renderer, solidWhiteTexture, Vertices.data(),
...
bool Waterrect::onDestroy()
{
    ...
    if (solidWhiteTexture)
    {
        SDL_DestroyTexture(solidWhiteTexture);

...
void Entity::Configure(const EntityInfo &eInfo)
{
    ...
    backpack =
        getServerWorldController().createBackpack(
...
void Entity::createRenderer()
{
    if (!renderer && info.EntityTypeID != 0)
    {
        ...
        widthRelative = 1.0f / *Eclipsea::GameSettings::getInstance().getRenderWidth();
        heightRelative = 1.0f / *Eclipsea::GameSettings::getInstance().getRenderHeight();
...
void Entity::Draw(float cameraX, float cameraY)
{
    if (drawable)
    {
        ...
        auto renderHeight = *Eclipsea::GameSettings::getInstance().getRenderHeight();

#include "Eclipsea/Gameplay/Entity/Entity.hpp"
...
void Entity::Configure(const EntityInfo &eInfo) {
  ...
  backpack = getServerWorldController().createBackpack(info.backpackCapacity);
...
void Entity::createRenderer() {
  if (!renderer && info.EntityTypeID != 0) {
    renderer =
        std::make_unique<Mob>(info.texture.meta.textureName,
                              info.texture.meta.cols, info.texture.meta.rows);
    renderer->Configure().Anchor(AnchorPoint::BottomCenter).Alpha(0.0f);

    widthRelative =
        1.0f / *Eclipsea::GameSettings::getInstance().getRenderWidth();
    heightRelative =
        1.0f / *Eclipsea::GameSettings::getInstance().getRenderHeight();
...
void Entity::Draw(float cameraX, float cameraY) {
  if (drawable) {
    ...
    auto renderWidth = *Eclipsea::GameSettings::getInstance().getRenderWidth();
    auto renderHeight =
        *Eclipsea::GameSettings::getInstance().getRenderHeight();
...
bool Entity::canMoveTo(const Vec3 &pos) const {
  auto &wc = getServerWorldController();

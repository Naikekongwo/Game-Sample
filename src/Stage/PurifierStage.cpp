...
void PurifierStage::onUpdate() {
  ...
  if (wrdController->queryHomelessItemInfo() != std::nullopt) {
    ...
    itemNamePtr->setText(std::format(
        "水箱容量：{:.0f}/{:.0f} 纯净水储量：{:.1f}", water, maxCap, purified));
  }
  ...
  // 根据容器中的物品更新净水器参数
  {
    ...
    if (nuclearSlot.has_value() && nuclearSlot->item.has_value()) {
      ...
      if (typeID >= 6 && typeID <= 8)
        purifyRate = static_cast<float>(typeID - 5); // 6→1.0, 7→2.0, 8→3.0
      else
        purifyRate = 1.0f;
    } else {
      purifyRate = 1.0f;
...
void PurifierStage::onRender() {
  ...
  if (item.has_value()) {
    ...
    ItemPickedUp->changeTexture(
        MakeTexture(meta->texture_cols, meta->texture_rows, meta->textureName));

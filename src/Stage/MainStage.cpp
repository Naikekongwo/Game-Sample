...
void MainStage::setupBackground()
{
    ...
    bg1->Animate()
        .Timer(5.0f)
        .Move(static_cast<uint16_t>(0.5f * Eclipsea::GameSettings::getInstance().getTargetWidth()),
              static_cast<uint16_t>(1.5f * Eclipsea::GameSettings::getInstance().getTargetHeight()),
              static_cast<uint16_t>(0.5f * Eclipsea::GameSettings::getInstance().getTargetWidth()),
              static_cast<uint16_t>(0.5f * Eclipsea::GameSettings::getInstance().getTargetHeight()),
              5.0f)
        .Commit();
...
void MainStage::setupButtons()
{
    // 创建按钮
    auto startButton =
        UI<Button>("startButton", 2, "img_StartButton", 1, 3);
    auto continueButton =
        UI<Button>("continueButton", 2, "img_ContButton", 1, 3);
    auto settingButton =
        UI<Button>("settingButton", 2, "img_SettButton", 1, 3);

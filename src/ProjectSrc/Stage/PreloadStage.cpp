...
void PreloadStage::onUpdate()
{
    ...
    if (preload && stageState == 2)
    {
        ...
        // auto oceanBackground = UI<ImageBoard>("background", 0,
        // background_ocean, 2, 1); 创建了加载界面的标题控件 创建了背景控件
        ...
        startTitle->Configure()
            ...
            .Sequence(false);
        // oceanBackground->Configure().Scale(fullwidth,
        // fullheight).Anchor(AnchorPoint::Center).Posite(0.5f, 0.28125f);
        startTitle->Animate().Fade(0.4f, 1.0f, 5.0f, false).Commit();
        // oceanBackground->Animate().Fade(0.0f, 1.0f, 10.0f,
        // false).Timer(5.0f).Fade(1.0f, 0.0f, 5.0f, false).Commit();

        // 使得背景的动画执行顺序变为顺序执行
        // oceanBackground->setSequential(true);

        auto Test = std::make_unique<Waterrect>(
            "water", 1,

...
bool Label::generateTexture(SDL_Texture *texture)
{
    ...
    SDL_Rect rect = getLogicalBounds();
    ...
    for (int i = 0; i <= AnimeState->frameIndex; i++)
    {
        ...
        SDL_Rect dstRect = {rect.x + currentCol * charWidth,
                            rect.y + currentRow * charHeight, charWidth,
                            charHeight};
        ...
        SDL_Surface *textSurface =
            TTF_RenderText_Blended(font, charStr, textColor);
        if (textSurface)
        {
            SDL_Texture *charTexture =
                SDL_CreateTextureFromSurface(GFXMgr.getRenderer(), textSurface);
            if (charTexture)
            {
                GFXMgr.RenderCopy(charTexture, NULL, &dstRect, 0, NULL,
                                  SDL_FLIP_NONE);
...
void Label::setText(const string &sentence, uint8_t fontWeight)
{
    ...
    for (int i = 0; i < fontGrid[1]; i++)
    {
        for (int j = 0; j < fontGrid[0]; j++)
        {
            ...
            SDL_Color textColor = {255, 255, 255, 255};
            char charStr[2] = {sentence[charIndex],
                               '\0'}; // 修复：创建以null结尾的字符串
            ...
            if (textSurface)
            {
                SDL_Texture *charTexture = SDL_CreateTextureFromSurface(
                    GFXMgr.getRenderer(), textSurface);
                if (charTexture)
                {
                    SDL_Rect dstRect = {j * charWidth, i * charHeight,
